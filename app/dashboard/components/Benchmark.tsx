...
import { Card, Legend, AreaChart, Title,Subtitle, Table, TableHead, TableHeaderCell, TableRow, TableBody, TableCell, Text, Badge} from '@tremor/react';
...
interface ueqBenchmark {
    Scale: "Attractiveness"|"Perspicuity"|"Efficiency"|"Dependability"|"Stimulation"|"Novelty",
    "25%": number,
    "50%": number,
    "75%": number,
    "90%": number,
...
export default function Benchmark({data}:{data:UEQ[]}) {
    ...
    function compareBenchmark(name:string, value:number): {name:string, ColorIndicator:string, Interpretation: string }{
        const benchmarkData = {
            "Sangat Baik": {
              name:"Sangat Baik",
    ...
    <Card>
        ...
        <Subtitle className='text-justify'>Rata-rata skala yang diukur ditetapkan dalam kaitannya dengan nilai-nilai yang ada dari kumpulan data benchmark. Kumpulan data ini berisi data dari 21.175 orang dari 468 penelitian mengenai berbagai produk (perangkat lunak bisnis, halaman web, toko web, jejaring sosial).
Perbandingan hasil produk yang dievaluasi dengan data dalam benchmark memungkinkan kesimpulan tentang kualitas relatif produk yang dievaluasi dibandingkan dengan produk lain.</Subtitle>
        ...
        <TableBody>
            ...
            <TableRow key={item.name}>
                ...
                <TableCell><Text>{item.value.toFixed(2)}</Text></TableCell>
                <TableCell><Badge color={compareBenchmark(item.name,item.value).ColorIndicator as "emerald" | "green" | "lime" | "yellow" | "red"}>{compareBenchmark(item.name,item.value).name}</Badge></TableCell>
                <TableCell>{compareBenchmark(item.name,item.value).Interpretation}</TableCell>
            </TableRow>
            ))}
        </TableBody>
        </Table>
    ...
    <Legend
      categories={["Buruk", "Dibawah rata-rata", "Diatas rata-rata", "Baik", "Sangat Baik"]}
    ...
    <AreaChart
      ...
      data={barChartdata}
      index="Scale"
      categories={["Bad", "BelowAverage", "AboveAverage", "Good", "Excellent"]}
      colors={["red", "yellow", "lime", "green","emerald"]}
