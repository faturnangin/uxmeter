...
export default function SiaUMP() {
  ...
  let [dialogHint, setDialogHint] = useState(false);
  const closeHint = () => setDialogHint(false);
  const openModal = () => {
    setIsModalOpen(true);
  };
  ...
  return (
    <>
      <section className="py-4 px-4 lg:px-24 lg:py-16">
        <div className="container mx-auto lg:flex lg:flex-row lg:items-center">
          ...
          <div className="py-4 flex flex-col gap-y-4 lg:gap-y-5 lg:w-1/2">
            <h1 className="text-2xl lg:text-4xl lg:text-start lg:leading-10 font-bold text-center text-transparent bg-gradient-to-r from-primary to-accent bg-clip-text">
              Evaluasi UX Website Sistem Informasi Akademik (SIA) Mahasiswa UMP
            </h1>
            <p className="text-base lg:text-lg lg:text-start font-medium text-textcolor text-justify">
              SIA UMP merupakan tempat pelayanan akademik bagi mahasiswa
            ...
            <ul className="space-y-2 text-base font-normal">
              <li className="flex space-x-2">
                ...
                <span className="text-textcolor/80">
                  Responden adalah mahasiswa aktif UMP
                </span>
              </li>
              <li className="flex space-x-2">
                <img src="/checkmark.svg" alt="check" />
                <span className="text-textcolor/80">
                  Dilakukan pada tahun ajaran 2023/2024
                </span>
              </li>
              <li className="flex space-x-2">
                <img src="/checkmark.svg" alt="check" />
            ...
            <button className="px-6 py-2 bg-primary hover:bg-secondary hover:text-textcolor rounded-full text-base lg:text-lg font-medium text-white">
              <Link href="/ueqsia#kuisioner">Mulai isi Kuisioner</Link>
            </button>
            <button
              onClick={openModal}
              className="px-6 py-2 bg-accent hover:bg-secondary hover:text-textcolor rounded-full text-base lg:text-lg font-medium text-white"
            >
              Petunjuk Pengisian
            </button>
          </div>
      ...
      <section id="kuisioner" className="mx-auto py-4 px-4 lg:px-24 lg:py-8">
        <h1 className="text-base lg:text-lg font-semibold mb-4">
          Kuisioner UEQ
        </h1>
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-12 gap-x-6 gap-y-6 justify-between">
            <div className="w-full col-span-12 lg:col-span-5 lg:h-fit border border-secondary bg-white shadow-md rounded-lg p-4 lg:p-6 overflow-hidden relative">
              <div className="absolute top-0 left-0 w-full px-4 py-2 lg:px-6 bg-primary text-white text-base lg:text-lg font-semibold">
                Informasi Pribadi
              </div>
              <div className="mt-8 flex flex-col gap-y-2">
                <label
                  htmlFor="name"
                  className="block text-gray-600 font-normal"
                >
                  Nama Lengkap
                </label>
                <input
            ...
            <div className="w-full col-span-12 lg:col-span-7 border border-secondary bg-white shadow-md rounded-lg p-4 lg:p-6 overflow-hidden relative">
              <div className="absolute top-0 left-0 w-full px-4 py-2 lg:px-6 bg-accent text-white text-base lg:text-lg font-semibold">
                Isi Kuisioner
              </div>
              {/* UEQ Starts */}
              {ueqparams.map((ueqparam, index) => (
                <div key={index} className="mt-8 flex flex-col gap-y-2">
                  <div className="flex items-center justify-between">
                    <h1>
                      {ueqparam.id}. {ueqparam.question}
                    </h1>
                    <Button
                      id="hint"
                      // onClick={() => handleHint(ueqparam.hint)}
                      onClick={(e) => {
                        e.preventDefault(); // Prevent default behavior
                        handleHint(ueqparam.hint);
                      }}
                          ...
                          <label className="inline-flex items-center">
                            <input
                              ...
                              checked={
                                parseInt(
                                  formData[
                                    `answer${ueqparam.id}` as keyof typeof formData
                                  ]
                                ) === value
                              }
                              onChange={handleInputChange}
          ...
          <div className="flex justify-end mt-4">
            <Button
              className="bg-primary text-white px-4 py-2 hover:bg-accent"
              type="submit"
              size="md"
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal}>
        <div className="p-6 w-full max-h-[80vh] overflow-y-scroll">
          ...
          <Text className="text-justify">
            Terima kasih telah bersedia berpartisipasi dalam penelitian kami
            tentang pengalaman pengguna. Kuisioner ini dirancang untuk
            mengumpulkan pandangan Anda tentang produk atau layanan yang telah
            Anda gunakan. Mohon luangkan waktu Anda untuk menjawab pertanyaan
            dengan jujur. Jawaban Anda sangat berharga bagi kami.
          </Text>
          ...
          <div className="mt-2">
            ...
            <Text>
              2. Pilih salah satu jawaban yang paling sesuai dengan pengalaman
              Anda.
            </Text>
          </div>
          <Subtitle className="mt-2">Skala Penilaian</Subtitle>
          <Text className="mt-2 text-justify">
            Anda akan melihat 26 indikator pernyataan, dan Anda diminta untuk
            menilai sejauh mana Anda setuju atau tidak setuju dengan
            masing-masing pernyataan. Harap perhatikan skala yang terletak di
            bagian kanan dan kiri peryataan, posisi skala tidak konsisten
            sehingga nilai negatif tidak selalu terletak di bagian kiri dan juga
            sebaliknya.
          </Text>
          ...
          <Card className="mt-2">
            <div className="flex flex-col gap-y-2">
              <h1 className="text-sm text-slate-500">
                1. Apakah Sistem Informasi Akademik Mahasiswa UMP
              </h1>
              <div className="flex justify-between items-center">
          ...
          <h1 className="mt-2 text-sm text-slate-500">
            Pada pertanyaan nomor 1 diatas menunjukan pengguna sangat setuju
            bahwa aplikasi yang diuji menyenangkan
          </h1>
        </div>
      </Modal>

      <ModalHint hint={hint} dialogHint={dialogHint} closeHint={closeHint} />

      {showSuccessCallout && (
        <CallOut
          title="Berhasil"
          message="Data telah dikirim, terima kasih atas partisi anda"
          isSuccess={true}
        />
      )}
      {showErrorCallout && (
        <CallOut
          title="Gagal"
          message="Data gagal dikirim, hubungi kontak bantuan"
          isSuccess={false}
        />
      )}
    </>
